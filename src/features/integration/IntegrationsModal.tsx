...
// const IntegrationsModal: React.FC<Props> = ({ open, onClose, onAmazonClick }) =>
//   const [mounted, setMounted] = useState(false);
//   const router = useRouter();
//   const reduxToken = useSelector((state: any) => state.auth?.token);
//   const [shopifyStore, setShopifyStore] = useState<any | null>(null);
...
// const handleChoose = (key: Provider) => {
//   // 🟢 AMAZON (HEADER) → open AmazonConnectLegacy via parent
//   if (key === "amazon") {
//     onAmazonClick?.();
//     return;
//   }
...
//   // Default behavior (for Shopify not connected): delegate to IntegrationDashboard
//   window.dispatchEvent(
//     new CustomEvent("integration:choose", {
//       detail: { provider: key, origin: "header" },
//     })
...
const IntegrationsModal: React.FC<Props> = ({
  ...
  //   if (key === "amazon") {
  //     onAmazonClick?.();
  //     onClose(); // close this modal as well
  //     return;
  //   }
  ...
  const handleChoose = (key: Provider) => {
    ...
    if (key === "shopify") {
      // ✅ already connected → go straight to Orders
