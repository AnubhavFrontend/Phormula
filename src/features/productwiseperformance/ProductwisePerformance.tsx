...
const ProductwisePerformance: React.FC<ProductwisePerformanceProps> = ({
    ...
    const fetchProductData = async () => {
        ...
        try {
            ...
            const payload = {
                product_name: productname,
                time_range: timeRange, // still Yearly by default
                year: selectedYear,
                quarter: timeRange === "Quarterly" ? selectedQuarter : null,
                countries,
            };
    ...
    useEffect(() => {
        fetchProductData();
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [productname, selectedYear, timeRange, selectedQuarter, selectedCountries]);
            ...
            <div className="mb-5">
                <div className="flex flex-col md:flex-row items-center justify-between gap-[0.5vw]">
                    <MonthYearPickerTable
                        ...
                        onYearChange={(val) =>
                            setSelectedYear(
                                Number(val) || initialYear
                            )
                        }
                    // if you want lowercase month values in state:
                    // valueMode="lower"
                    />
                </div>
            </div>
