...
// //     // if it's just spaces or empty after decoding, ignore it
// //     if (!decoded.trim()) return undefined;

// //     return fromSlug(decoded);
// //   } catch {
// //     // fallback: if decodeURIComponent fails, still try fromSlug
...
// //   // ❌ removed "Menthol" default – empty until user selects from dropdown / URL
...
// //     if (!canShowResults) return; // safety guard
...
// //   useEffect(() => {
// //     if (!canShowResults) return; // ❗ don't fetch until filters + product are set
// //     fetchProductData();
...
// //         .map((country) => makeDataset(country, metric, suffix));
...
// //       };
// //     });
// //   }, [data, monthOrder, GBP_TO_USD_RATE]);
...
// //           <PeriodFiltersTable
// //             range={range}
// //             selectedMonth={selectedMonth} // still required, but won't be used if monthly is hidden
...
// //       {/* Loading – only after all selections done */}
...
// //       {/* Error – only after all selections done */}
...
// //       {/* Results – only when selections are complete */}
...
//   // this is just the UI toggle state; rendering will be restricted by countriesFromApi
//   const [selectedCountries, setSelectedCountries] = useState<
//     Record<CountryKey, boolean>
//   >({
//     uk: true,
...
//   const handleCountryChange = (country: CountryKey) => {
//     if (country === "global") return; // don't toggle global

//     setSelectedCountries((prev) => ({
//       ...prev,
//       [country]: !(prev[country] ?? true), // missing means true by default
//     }));
//   };
...
//   // Countries actually returned by the API
//   const countriesFromApi = useMemo(
//     () => (data?.data ? (Object.keys(data.data) as CountryKey[]) : []),
//     [data]
//   );
...
//       .filter(([country, rows]) => {
//         if (country.toLowerCase() === "global") return false; // handle global separately
//         return rows.some(
//           (m) =>
//             m.net_sales !== 0 ||
//             m.quantity !== 0 ||
...
//     const charts = metrics.map(({ metric, suffix }) => {
//       // ✅ Global always, plus only countries that have data AND are toggled on
...
//                       type="button"
//                       onClick={
//                         isGlobal ? undefined : () => handleCountryChange(country)
//                       }
//                       className={`flex items-center gap-2 rounded-full px-2 py-1 text-sm font-semibold border transition
//         ${isSelected
//                           ? "bg-white border-transparent"
...
//           {/* Summary Cards – global always, others if toggled & present */}
//           <div className="mt-8">
...
//       if (!isGlobal && !(selectedCountries[key] ?? true)) return null;
//       if (
//         !isGlobal &&
