...
const Bargraph: React.FC<BargraphProps> = ({
  ...
  return (
    <div className="relative w-full">
      ...
      {/* Legend / toggle group in a single horizontal row */}
      {/* <div
        className={[
          "w-full mt-2 overflow-x-auto",
          allValuesZero ? "opacity-30" : "opacity-100",
          "transition-opacity duration-300",
        ].join(" ")}
      >
        <div className="flex flex-nowrap items-center gap-4 min-w-max pr-4">
          {[
            { name: "sales", label: "Sales", color: "#2CA9E0" },
            { name: "total_cous", label: "COGS", color: "#AB64B5" },
            { name: "AmazonExpense", label: "Amazon Fees", color: "#ff5c5c" },
            ...
            const isOnlyOneLeft =
              checked &&
              Object.values(selectedGraphs).filter(Boolean).length === 1;

            return (
              <label
                key={name}
                className="shrink-0 flex items-center gap-2 text-[13px] sm:text-[14px] md:text-[15px] font-medium select-none whitespace-nowrap"
                style={{ color }}
              >
                <input
                  type="checkbox"
