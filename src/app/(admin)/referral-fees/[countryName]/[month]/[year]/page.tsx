...
// //       // 👉 build query params with country, month, year
...
// // generic sku-wise row (dynamic columns from DB)
// type SkuWiseRow = Record<string, any>;
...
//   });
//   const [feeSummaryRows, setFeeSummaryRows] = useState<FeeSummaryRow[]>([]);
//   const [skuWiseRows, setSkuWiseRows] = useState<SkuWiseRow[]>([]); // ⭐ NEW
//   const [loading, setLoading] = useState(false);
//   const [error, setError] = useState<string | null>(null);
//   const [userId, setUserId] = useState<string>("unknown");
...
//       // ⭐ NEW: sku-wise table from backend
//       const skuWiseTable = json?.skuwise_table_data ?? [];
//       const skuArr: SkuWiseRow[] = Array.isArray(skuWiseTable)
//         ? skuWiseTable
//         : [];
...
//   // Helper for SKU-wise dynamic columns
//   const skuWiseColumns = useMemo(() => {
//     if (!skuWiseRows.length) return [];
//     return Object.keys(skuWiseRows[0]);
//   }, [skuWiseRows]);
...
//       {/* ⭐ SKU-wise Monthly Summary (new) */}
//     {/* ✅ SKU Level Referral Fees Table – no-wrap headers + horizontal scroll */}
//       <div className="bg-white rounded-2xl shadow p-4 w-full overflow-x-auto">
//         <div className="flex items-center justify-between mb-2 gap-2 min-w-max">
//           <h3 className="text-sm font-semibold">
//             SKU Level Referral Fees Details
//           </h3>
//           <button
//             onClick={handleDownloadExcel}
...
//                 Category
//               </th>
//               <th className="text-left py-2 px-2 whitespace-nowrap">ASIN</th>
  ...
  return (
    <div className="p-4 space-y-4 font-sans text-[#414042]">
      ...
      <div className="bg-white rounded-2xl shadow p-4 w-full overflow-x-auto">
        <div className="flex items-center justify-between mb-2 gap-2 min-w-max">
          <h3 className="text-sm font-semibold">
            SKU Level Referral Fees Details (Monthly SKU-wise)
          </h3>
          <button
            onClick={handleDownloadExcel}
            className="inline-flex items-center gap-2 bg-gray-800 text-white px-3 py-2 rounded-xl text-xs md:text-sm whitespace-nowrap"
            ...
            {skuwiseRows.map((r, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="py-2 px-2 whitespace-nowrap">{r.asin}</td>
                  <td className="py-2 px-2 whitespace-nowrap">
                    {fmtNumber(quantity)}
                  </td>
                  <td className="py-2 px-2 whitespace-nowrap">
