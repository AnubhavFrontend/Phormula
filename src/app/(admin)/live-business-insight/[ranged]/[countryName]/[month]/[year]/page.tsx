...
const segmentTotal = segmentTotalsMap[activeTab] ?? null;
            ...
            >
              {['top_80_skus', 'new_or_reviving_skus', 'other_skus'].map((key) => (
                <button
                  key={key}
                  onClick={() =>
                    setActiveTab(key as keyof CategorizedGrowth)
                  }
 ...
 {hasAnySkus ? (
  ...
  <tr className="bg-[#D9D9D9E5] ">
    ...
    <td className="border px-2 py-2.5 font-bold text-center">
      {segmentTotal && segmentTotal['Sales Mix (Month2)'] != null
        ? `${Number(segmentTotal['Sales Mix (Month2)']).toFixed(2)}%`
        : activeTab === 'new_or_reviving_skus'
    ...
    {[
      ...
      if (activeTab === 'new_or_reviving_skus' && key) {
        return (
          <td
            key={idx}
            className="border px-2 py-2.5 font-bold text-center"
          >
            {Number(
              (manualTotalsForNewRev as any)[key] ?? 0
            ).toFixed(2)}
          </td>
        );
      }
