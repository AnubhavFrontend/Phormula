...
//   defaultRegion?: RegionKey;
// }) {
//   // figure out which regions are actually "connected" (non-zero)
//   const availableRegions = useMemo<RegionKey[]>(() => {
//     const entries = Object.entries(regions) as [RegionKey, RegionMetrics][];
//     return entries
//       .filter(([key, val]) =>
//         key === "Global" ? true : !!(val.mtdUSD || val.lastMonthTotalUSD || val.targetUSD)
//       )
//       .map(([key]) => key);
//   }, [regions]);

//   const initialTab: RegionKey =
//     availableRegions.includes(defaultRegion) ? defaultRegion : "Global";

//   const [tab, setTab] = useState<RegionKey>(initialTab);

//   // keep tab in sync if availableRegions change
//   useEffect(() => {
//     if (!availableRegions.includes(tab)) {
//       setTab(initialTab);
//     }
//   }, [availableRegions, initialTab, tab]);
...
//   // which region tab is selected in the Amazon card (for UI only)
//   const [amazonRegion, setAmazonRegion] = useState<RegionKey>("UK");
...
//   // Regions for the gauge + "connected" info (Global, UK, US, CA)
//   const regions = useMemo(
//     () => {
//       const globalLastMonthTotal = chooseLastMonthTotal(
//         MANUAL_LAST_MONTH_USD_GLOBAL,
...
//   // Regions that are actually "connected" (non-zero), excluding Global
//   const connectedAmazonRegions = useMemo<RegionKey[]>(() => {
//     const order: RegionKey[] = ["UK", "US", "CA"];
//     return order.filter((key) => {
//       const r = regions[key];
//       if (!r) return false;
//       return !!(r.mtdUSD || r.lastMonthTotalUSD || r.targetUSD);
//     });
//   }, [regions]);

//   // Global metrics card (left top) – combined Amazon + Shopify, in USD
//   const globalMetrics = useMemo(() => {
//     const amazonUnits = toNumberSafe(cms?.total_quantity ?? 0);
//     const shopifyUnits = toNumberSafe(shopifyDeriv?.totalOrders ?? 0);
//     const units = amazonUnits + shopifyUnits;

//     const amazonSalesUSD = amazonUK_USD;
//     const shopifySalesUSD = toNumberSafe(shopifyDeriv?.netSales) * INR_TO_USD;
//     const salesUSD = amazonSalesUSD + shopifySalesUSD;

//     const aspUSD = units > 0 ? salesUSD / units : 0;

//     const profitAmazonUSD = toNumberSafe(uk.profitGBP) * GBP_TO_USD;
//     const profitUSD = profitAmazonUSD; // we only have Amazon profit for now
//     const profitPct = salesUSD > 0 ? (profitUSD / salesUSD) * 100 : 0;

//     return { units, salesUSD, aspUSD, profitUSD, profitPct };
//   }, [cms?.total_quantity, shopifyDeriv, amazonUK_USD, uk.profitGBP]);
...
//       <div className="grid grid-cols-12 gap-6">
//         {/* LEFT 8: Global + Amazon + Shopify */}
//         <div className="col-span-12 lg:col-span-8 space-y-6">
//           {/* GLOBAL card (combined view) */}
//           <div className="rounded-2xl border bg-white p-5 shadow-sm">
...
//                 <div className="mt-2 text-lg font-semibold">
//                   <ValueOrSkeleton loading={anyLoading} mode="inline" compact>
//                     {fmtUSD(globalMetrics.profitUSD)}
//                   </ValueOrSkeleton>
//                 </div>
//               </div>
...
//                 <div className="mt-2 text-lg font-semibold">
//                   <ValueOrSkeleton loading={anyLoading} mode="inline" compact>
//                     {fmtPct(globalMetrics.profitPct)}
//                   </ValueOrSkeleton>
//                 </div>
//               </div>
//             </div>
...
//               {/* Region toggle – only show connected ones, like UK now */}
//               <div className="inline-flex rounded-lg border bg-gray-50 p-1 text-xs w-full sm:w-auto justify-between sm:justify-start">
//                 {connectedAmazonRegions.map((key) => (
//                   <button
//                     key={key}
//                     type="button"
//                     onClick={() => setAmazonRegion(key)}
...
//         {/* RIGHT 4: Sales Target card (gauge) */}
//         <aside className="col-span-12 lg:col-span-4">
//           <div className="lg:sticky lg:top-6">
//             <SalesTargetCard regions={regions} defaultRegion="Global" />
//           </div>
